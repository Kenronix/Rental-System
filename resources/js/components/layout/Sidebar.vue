...
const navigationItems = [
  { name: 'Dashboard', icon: HomeIcon, path: '/landlord/dashboard' },
  { name: 'Properties', icon: BuildingOfficeIcon, path: '/landlord/properties' },
  { name: 'Tenants', icon: UsersIcon, path: '/landlord/tenants' },
  { name: 'Payments', icon: CreditCardIcon, path: '/landlord/payments' },
  { name: 'Reports', icon: ChartBarIcon, path: '/landlord/reports' }
]
...
<template>
  <div class="sidebar" :style="{ backgroundImage: `url(${sidebarBg})` }">
    ...
    <div class="user-profile">
      <div class="avatar">{{ user ? user.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) : 'U' }}</div>
      <div class="user-info">
